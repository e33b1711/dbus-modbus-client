...
class ModbusDevice(object):
    ...
    def read_data_regs(self, regs, d):
        ...
        #print("reading:")
        for reg in regs:
            #print(reg.name + " " + str(reg.time) + " " + str(reg.max_age))
            base  = reg.base
            count = reg.count

            if now - reg.time > reg.max_age:
                rr = self.modbus.read_holding_registers(base, count, unit=self.unit)
                #print("read " + str(base) + " " + str(count))
                if reg.decode(rr.registers):
